# ⚡ 快速开始

## 🚀 5步开始使用

### Step 1: 配置Claude Projects（5分钟）

```bash
1. 访问 https://claude.ai/projects
2. 创建新项目："思考，推理，读书"
3. 查看详细步骤：claude-projects/快速开始.md
```

### Step 2: 选择第一本书（1分钟）

- [创业者书单（50本）](book-lists/创业者书单-50本.md)
- [投资者书单（52本）](book-lists/投资者书单-52本.md)

**推荐第一本书**：
- 创业者 → 《从0到1》
- 投资者 → 《巴菲特致股东信》
- 管理者 → 《原则》

### Step 3: 开始阅读（30-60分钟/天）

在Claude Projects中输入：

```
我想读《从0到1》
```

### Step 4: 记录笔记（边读边记）

选择合适的模板：
- [理论类书籍模板](reading-notes/templates/理论类书籍模板.md)
- [案例类书籍模板](reading-notes/templates/案例类书籍模板.md)
- [传记类书籍模板](reading-notes/templates/传记类书籍模板.md)

### Step 5: 实践应用（持续）

记录行动日志：
- [行动日志模板](action-logs/template-行动日志模板.md)

---

## 📋 常用触发词

在Claude Projects对话中使用：

| 触发词 | 作用 |
|--------|------|
| `【继续】` | 继续下一段对话 |
| `【暂停】` | 暂停当前单元，稍后继续 |
| `【跳过】` | 跳过当前段落 |
| `【提交答案】` | 提交练习题答案 |
| `【总结】` | 生成总结和智慧卡片 |

---

## 📂 文件路径速查

| 目录 | 作用 | 路径 |
|------|------|------|
| 书单 | 选择要读的书 | `book-lists/` |
| 笔记模板 | 记录读书笔记 | `reading-notes/templates/` |
| 智慧卡片 | 提炼核心观点 | `knowledge-cards/` |
| 练习题 | 深度思考练习 | `practice-exercises/` |
| 行动日志 | 记录实践过程 | `action-logs/` |
| 配置指南 | Claude Projects配置 | `claude-projects/` |
| 使用文档 | 详细使用说明 | `docs/` |

---

## 🔗 关键链接

- [完整使用指南](docs/如何使用本仓库.md)
- [常见问题FAQ](docs/常见问题FAQ.md)
- [Claude Projects配置](claude-projects/配置指南.md)
- [三湘问道知识库](https://github.com/MRYGP/sanxiangwendao)

---

## 💡 快速提示

### 第一次使用
1. 先读 [5分钟快速开始](claude-projects/快速开始.md)
2. 配置Claude Projects
3. 从一本简单的书开始（如《从0到1》）

### 遇到问题
1. 查看 [常见问题FAQ](docs/常见问题FAQ.md)
2. 检查Claude Projects配置是否正确
3. 确认Project Knowledge是否添加了sanxiangwendao文档

### 提升效率
1. 使用模板记录笔记（不要从头写）
2. 边读边记，不要事后补
3. 定期回顾（每周/每月）

---

**更新日期**：2025-01-11  
**维护者**：松阳
